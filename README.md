...
`run_review.py` loads `.env` automatically via `python-dotenv` — no `export` step needed. Values already exported in your shell still take precedence over `.env`, so `--provider`/`--model`/`--base-url` flags and one-off `export FOO=bar` overrides work exactly as before.
